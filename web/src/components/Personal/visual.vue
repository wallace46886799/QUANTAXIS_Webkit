...
export default {
  ...
  methods: {
    ...
    ready() {
      // 先从ready拿到信息数据
      const val = this.data0
      axios
        .get('http://localhost:3000/backtest/info_cookie?cookie=' + val)
        .then(response => {
          ...
          this.items[0]['annualized_returns'] = data[
            'annualized_returns'
          ].toFixed(3)
          this.items[0]['benchmark_annualized_returns'] = data[
